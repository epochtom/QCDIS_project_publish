...
# ============================= CONFIG =============================
class CFG:
    DATASET_PATH     = "/content/dataset.csv"      # your CSV
    TARGET_COLUMN    = None                     # auto-detect
    TEST_SIZE        = 0.2
    RANDOM_STATE     = 42
    N_QUBITS         = 16                       # Increased from 4 to 16
    N_COMPONENTS     = 16                       # Changed from 4 to 16
    BATCH_SIZE       = 32
...
def projected_quantum_features(X: np.ndarray, n_components: int, n_qubits: int) -> np.ndarray:
    """Projected Quantum Kernel features via measurements."""
    n_samples, n_features = X.shape
    n_components = min(n_components, n_qubits, n_features)
    ...
    def circuit(x):
        embedding(x)
        return [qml.expval(qml.PauliZ(i)) for i in range(n_components)]

    # ---- Compute features for each sample ----
    features = np.zeros((n_samples, n_components))
    for i in range(n_samples):
        features[i] = circuit(X_norm[i])

    return features


def safe_pqk(X, n_components, n_qubits):
    """Wrap PQK with a classical PCA fallback."""
    try:
        print(f"  → Projected Quantum Kernel ({n_qubits} qubits, {n_components} components)...")
        return projected_quantum_features(X, n_components, n_qubits)
    except Exception as e:
        print(f"  → PQK failed ({e}), falling back to classical PCA")
...
def main():
    ...
    print("="*70)
    print(" PQK → 1-D CNN CLASSIFIER + PLOT ".center(70))
    print("="*70)
    ...
    if CFG.FORCE_CLASSICAL:
        ...
        X_tr_q = safe_pqk(X_tr_s, CFG.N_COMPONENTS, CFG.N_QUBITS)
        X_te_q = safe_pqk(X_te_s, CFG.N_COMPONENTS, CFG.N_QUBITS)
    ...
    print(f"MODEL: ProjectedQuantumKernel({CFG.N_QUBITS}qb) → 1-D CNN")
    print("="*70)
