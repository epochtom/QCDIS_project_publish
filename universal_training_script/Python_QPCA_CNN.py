...
from sklearn.decomposition import PCA                     # <-- classical fallback
...
class CFG:
    ...
    TEST_SIZE        = 0.2
    RANDOM_STATE     = 42
    N_QUBITS         = 4                       # any integer >= 1 and >= N_COMPONENTS
    N_COMPONENTS     = 4                       # QPCA output dim
    N_SUBSET         = 8                        # subset size for kernel
    BATCH_SIZE       = 32
    EPOCHS           = 20
    LR               = 1e-3
    JITTER           = 1e-6
...
# ========================= QUANTUM KERNEL PCA =========================
def quantum_kernel_pca(X: np.ndarray, n_components: int, n_qubits: int, subset_size: int) -> np.ndarray:
    """Quantum Kernel PCA with a small random subset (stable & fast)."""
    ...
    # ---- Build kernel on a random subset ----
    ...
    # ---- Project full data onto eigenvectors ----
    K_full = np.zeros((n_samples, subset_size))
    for i in range(n_samples):
        for j in range(subset_size):
            val = kernel(X_norm[i], X_sub[j])
    ...
    return K_full @ eigvecs                     # (n_samples, n_components)


def safe_qpca(X, n_components, n_qubits, subset_size):
    """Wrap QPCA with a classical PCA fallback."""
    try:
        print(f"  → Quantum Kernel PCA ({n_qubits} qubits, {n_components} components, subset={subset_size})...")
        return quantum_kernel_pca(X, n_components, n_qubits, subset_size)
    except Exception as e:
        print(f"  → QPCA failed ({e}), falling back to classical PCA")
...
class QuantumCNN(nn.Module):
    def __init__(self, n_features: int, n_classes: int):
        ...
        with torch.no_grad():
            dummy = torch.zeros(1, 1, n_features)          # (B, C, L)
            out = self.conv(dummy)
...
def main():
    ...
    print("="*70)
    print(" QPCA → 1-D CNN CLASSIFIER + PLOT ".center(70))
    print("="*70)
    ...
    if CFG.FORCE_CLASSICAL:
        ...
        print("  → Classical mode (PCA only)")
    else:
        X_tr_q = safe_qpca(X_tr_s, CFG.N_COMPONENTS, CFG.N_QUBITS, CFG.N_SUBSET)
        X_te_q = safe_qpca(X_te_s, CFG.N_COMPONENTS, CFG.N_QUBITS, CFG.N_SUBSET)
    ...
    model = QuantumCNN(n_features=CFG.N_COMPONENTS, n_classes=n_classes).to(device)
    ...
    print(f"MODEL: QuantumKernelPCA({CFG.N_QUBITS}qb) → 1-D CNN")
    print("="*70)
